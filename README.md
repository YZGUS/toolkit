...
├── grok/                          # Grok 对话
│   ├── multi-turn.mjs
│   ├── modes-test.mjs             # Fast / Expert 模式回归
...
node bin/grok-ask.mjs --mode=fast "你好"
node bin/grok-ask.mjs --mode=expert "推理：A>B, B>C, A 与 C 的关系？"
...
```js
import {
  askGrok, createGrokChat, GROK_MODES,
...
// Grok - Fast / Expert 模式
await askGrok('简单问题', { mode: 'fast' });
await askGrok('深度推理一下', { mode: 'expert' });

// Grok 多轮，可中途切换模式
const gChat = await createGrokChat({ mode: 'fast' });
await gChat.send('你好');
await gChat.setMode('expert');
await gChat.send('换个深度问题');
await gChat.close();
...
Grok 模型选择是 Radix DropdownMenu（trigger `#model-select-trigger`），下拉里有 Auto / Fast / Expert / Heavy 四项。

- **激活态识别**：Grok 的 menuitem 不带 `aria-checked` / `data-state=checked`，但 trigger 按钮文本始终就是当前激活模式名（如 "Fast"），所以 `setGrokMode` 通过读 trigger 文本判定是否需要切换
- **点击模拟**：Radix 监听 `pointerdown` 事件，DOM `.click()` 不会触发菜单展开，必须用 `page.click()` 走完整鼠标事件
- **支持模式**：

| mode 值 | 名称 | 默认超时 |
|---|---|---|
| `default` | 不切换，沿用页面当前 trigger 状态 | 180s |
| `fast` | Fast（轻量任务） | 120s |
| `expert` | Expert（深度思考） | 300s |

Heavy（Team of Experts）需付费订阅，未纳入。

调用 `askGrok(msg, { mode: 'expert' })` 或 `chat.setMode('fast')` 即可。
